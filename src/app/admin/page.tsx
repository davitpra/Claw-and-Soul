...
export default function AdminOverviewPage() {
  ...
  useEffect(() => {
    adminApi.stats.overview().then(setStats).catch((e) => setError(e.message));
  }, []);
  ...
  if (error) {
    return (
      <div className="flex items-center gap-3 text-red-600 bg-red-50 rounded-xl p-4">
        ...
        <span className="text-sm font-semibold">Error al cargar estadísticas: {error}</span>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        ...
        <p className="text-text-muted text-sm mt-1">Métricas globales de la plataforma</p>
      </div>

      {/* KPI Cards */}
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
        <StatCard label="Usuarios" value={stats.totals.users} icon="group" sub={`${stats.usersByRole.admin ?? 0} admins`} />
        <StatCard label="Mascotas" value={stats.totals.pets} icon="pets" />
        <StatCard label="Generaciones" value={stats.totals.generations} icon="auto_awesome" sub={`${stats.generationsByStatus.failed ?? 0} fallidas`} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-2xl border border-[#E0DED9] p-6 shadow-sm">
          ...
          {stats.timeline.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={stats.timeline} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                ...
                />
                <Line type="monotone" dataKey="count" stroke="#448da6" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </div>

        <div className="bg-white rounded-2xl border border-[#E0DED9] p-6 shadow-sm">
          ...
          {statusPieData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie data={statusPieData} cx="50%" cy="50%" innerRadius={55} outerRadius={80} dataKey="value" paddingAngle={3}>
                  {statusPieData.map((entry) => (
                    <Cell key={entry.name} fill={STATUS_COLORS[entry.name] ?? "#94a3b8"} />
                  ))}
                </Pie>
                <Tooltip contentStyle={{ borderRadius: 8, border: "1px solid #E0DED9", fontSize: 12 }} />
      ...
      <div className="bg-white rounded-2xl border border-[#E0DED9] p-6 shadow-sm">
        ...
        {stats.topStyles.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={stats.topStyles} margin={{ top: 4, right: 8, left: -20, bottom: 40 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#E0DED9" />
              <XAxis
                dataKey="displayName"
                tick={{ fontSize: 11, fill: "#5c747c" }}
            ...
            {stats.recentSyncs.map((s) => (
              <div key={s.id} className="flex items-center justify-between py-3">
                <div className="flex items-center gap-3">
                  <span
                    className={`text-xs font-semibold px-2 py-0.5 rounded-full border ${SYNC_STATUS_BADGE[s.status] ?? "bg-gray-50 text-gray-700"}`}
                  >
                    {s.status}
                  </span>
                ...
                <div className="text-right">
                  ...
                  {s.productsChecked != null && (
                    <p className="text-xs text-text-muted">
                      {s.productsChecked} revisados · {s.productsCreated ?? 0} creados · {s.productsUpdated ?? 0} actualizados
                    </p>
                  )}
                </div>
              </div>
