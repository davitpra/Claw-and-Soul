 ...
 * the border segment midpoints, loop closed), scaled by `sizeMultiplier`. Returns
 * null when the facet has no border segments to trace.
 */
export function buildFacetPathData(f: Facet, sizeMultiplier: number = 1): string | null {
 ...
 * at a uniform target size (capped to fontSize; smaller facets shrink to fit) and
 * scaled by `sizeMultiplier`.
 */
export function buildFacetLabel(f: Facet, fontSize: number, fontColor: string, sizeMultiplier: number = 1): SVGGElement {
    ...
    public static async createSVG(facetResult: FacetResult, colorsByIndex: RGB[], sizeMultiplier: number, fill: boolean, stroke: boolean, addColorLabels: boolean, fontSize: number = 50, fontColor: string = "black", fillOpacity: number = 1, onUpdate: ((progress: number) => void) | null = null) {
        ...
        for (const f of facetResult.facets) {
            ...
            if (f != null && data != null) {
                ...
                if (addColorLabels) {
                    svg.appendChild(buildFacetLabel(f, fontSize, fontColor, sizeMultiplier));
                }
