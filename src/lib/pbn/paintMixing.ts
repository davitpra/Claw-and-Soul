...
import { rgbToPigment, pigmentToRgb, mixPigments, Pigment } from "./vendor/spectral";
...
function makeRecipe(
  paints: BasePaint[],
  parts: number[],
  paintPigments: Pigment[],
  targetLab: number[]
): MixRecipe {
  const total = parts.reduce((s, p) => s + p, 0);
  const weights = parts.map(p => p / total);
  const pigments = paints.map((_, i) => paintPigments[i]);
  const mixed = pigmentToRgb(mixPigments(pigments, weights));
...
export function findRecipe(target: RGB, opts: FindRecipeOptions = {}): MixRecipe {
  ...
  // Precompute pigments for all base paints
  const paintPigments: Pigment[] = paints.map(p => rgbToPigment(p.rgb));
  ...
  // Phase 0: singles
  for (let i = 0; i < K; i++) {
    const mixed = pigmentToRgb(paintPigments[i]);
    ...
    for (let j = i + 1; j < K; j++) {
      for (const [a, b] of PAIR_RATIOS) {
        const recipe = makeRecipe(
          [paints[i], paints[j]],
          [a, b],
          [paintPigments[i], paintPigments[j]],
          targetLab
        );
        ...
        for (let k = j + 1; k < K; k++) {
          for (const [a, b, c] of TRIPLE_RATIOS) {
            const recipe = makeRecipe(
              ...
              [a, b, c],
              [paintPigments[i], paintPigments[j], paintPigments[k]],
              targetLab
            );
